Shader "URPTraining/URPuvouttopixel"
{ 
   Properties {   	

     _MainTex("RGB 01", 2D) = "white" {}
    _MainTex02("RGB 02", 2D) = "white" {}
    __TintColor("color",color)=(1,1,1,1)//r,g,b,a
        ...
        float4 _MainTex02_ST;
         ...
         // float4 object does not have methods at line 106 (on metal)깜빡했더니난 오류 이거선언 텍스쳐
      	...
      	VertexOutput vert(VertexInput v)
        	{
          		VertexOutput o;      
                //각각의 텍스쳐의 움직임을 각기 제어할 수 있도록 
          		o.vertex = TransformObjectToHClip(v.vertex.xyz);//mvp변환만 
                //o.uv = v.uv.xy * _MainTex_ST.xy + _MainTex_ST.zw;//scale은 곱하고 / offset은 더해준다
